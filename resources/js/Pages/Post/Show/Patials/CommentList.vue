            ...
            <li class="border-b border-gray-300" v-for="item in data.data" :key="item.id">
                <div v-if="editableId!==item.id">
                    ...
                    <div v-else class="flex gap-1.5 py-1">
                        ...
                        <TextButton v-if="item.user.id===auth?.user?.id" title="수정" size="text-xs" @event="editableId = item.id"/>
                        <TextButton v-if="item.user.id===auth?.user?.id" title="삭제" size="text-xs" @event="emits('destroyComment', {id: item.id, post_id: route().params.id})"/>
                    ...
                    <div v-for="reply in item.replies" :key="reply.id" >
                        <ReplyItem v-if="editableId!==reply.id" :item="reply" :auth="auth">
                            <template v-slot:buttons>
                                ...
                                <TextButton class="ml-1" title="삭제" size="text-xs" @event="emits('destroyComment', {id: reply.id, post_id: route().params.id})"/>
                            </template>
                        </ReplyItem>
